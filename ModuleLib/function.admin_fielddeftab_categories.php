...
$fields = generator_tools::get_field_defs($this, null, null, null, 'categories');
$rowclass = 'row1';
$entryarray = array();
...
if (is_array($fields) && count($fields) > 0) {
    foreach ($fields as $row) {
        ...
        // move down
        /* if ($position > $onerow->position) {
          $onerow->downlink = $this->CreateLink($id, 'admin_movefielddef', $returnid, $admintheme->DisplayImage('icons/system/arrow-d.gif', $this->Lang('down'), '', '', 'systemicon'), array('fielddef_id' =>$row['fielddef_id'], 'dir' =>'down'));
          } else {
          $onerow->downlink = '';
          } */
...
$smarty->assign('section', 'categories');
